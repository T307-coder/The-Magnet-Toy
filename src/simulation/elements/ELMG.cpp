...
void Element::Element_ELMG()
{
	Identifier = "DEFAULT_PT_ELMG";
	Name = "ELMG";
	Colour = 0x0A5B8B_rgb;
	MenuVisible = 1;
...
static int update(UPDATE_FUNC_ARGS)
{
	...
	else
	{
		...
		sim->magSrc[y / CELL][x / CELL] = 0.2f * (parts[i].temp - 273.15);
...
static int graphics(GRAPHICS_FUNC_ARGS)
{
	// Blue when activated (life==10), dim when deactivated
	if (cpart->life == 10)
	{
		*colr = 30; *colg = 80; *colb = 200;
	}
	else
	{
		*colr = 10; *colg = 30; *colb = 80;
	}
	return 0;
}
