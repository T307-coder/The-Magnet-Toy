...
electricity_chargeContact(sim, parts[i], x, y, parts[i].tmp3);
	electricity_applyForce(sim, parts[i], x, y, parts[i].tmp3, 0.5f);
	electricity_diffuseCharge(sim, parts[i], x, y, parts[i].tmp3);
	electricity_applyLorentz(sim, parts[i], x, y, parts[i].tmp3);
	return 0;
}
