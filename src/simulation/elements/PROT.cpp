...
static int update(UPDATE_FUNC_ARGS)
{
	...
	// Magnetic Lorentz force: F = q(v x B), PROT is positive charge
	...
	// Electric field: PROT (positive) contributes to eSrc, feels F = q*E = -q*grad V
	if (sim->electricityEnabled)
	{
		...
		if (cx >= 0 && cy >= 0 && cx < XCELLS && cy < YCELLS)
		{
			...
			if (cx > 0 && cy > 0 && cx < XCELLS - 1 && cy < YCELLS - 1)
			{
				...
				// F = q*E = -q*grad V. PROT q=+1: F = -grad V
				parts[i].vx -= dEx * 0.5f;
				parts[i].vy -= dEy * 0.5f;
			}
		}
