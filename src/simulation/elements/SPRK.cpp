...
static int update(UPDATE_FUNC_ARGS)
{
	...
	if (parts[i].life<=0)
	{
		...
		if (sim->part_change_type(i,x,y,ct))
			return 1;
		// Induced SPRK (magnetic induction): short spark, keep tmp4 for restore
		if (parts[i].tmp2 == 1)
		{
			parts[i].life = 4;
		}
	...
	case PT_ETRD:
		if (parts[i].life==1)
		{
			...
			if (pavg != PT_INSL && pavg != PT_RSSS)
			{
				...
				if (parts[i].tmp2 == 1) parts[nearp].tmp2 = 1;
			}
		}
		break;
	case PT_NBLE:
			...
			if (rx || ry)
			{
				...
				if (receiver==PT_WATR||receiver==PT_SLTW) {
					if (parts[ID(r)].life==0 && parts[i].life<3)
					{
						...
						if (parts[i].tmp2 == 1) parts[ID(r)].tmp2 = 1;
					}
				}
				...
				if (sim->magnetismEnabled && sim->sprkCurrentEnabled && parts[i].tmp2 != 1)
				{
