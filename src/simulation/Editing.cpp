...
std::unique_ptr<Snapshot> Simulation::CreateSnapshot() const
{
	...
	snap->EField    .insert(snap->EField    .begin(), &eField   [0][0], &eField   [0][0] + NCELL);
	snap->BField    .insert(snap->BField    .begin(), &bField   [0][0], &bField   [0][0] + NCELL);
	snap->PrevEField.insert(snap->PrevEField.begin(), &prevEField[0][0], &prevEField[0][0] + NCELL);
	snap->PrevBField.insert(snap->PrevBField.begin(), &prevBField[0][0], &prevBField[0][0] + NCELL);
	snap->ESrc      .insert(snap->ESrc      .begin(), &eSrc      [0][0], &eSrc      [0][0] + NCELL);
	snap->MagSrc    .insert(snap->MagSrc    .begin(), &magSrc    [0][0], &magSrc    [0][0] + NCELL);
	snap->signs = signs;
...
void Simulation::Restore(const Snapshot &snap)
{
	...
	player2 = snap.stickmen[snap.stickmen.size() - 2];
	{
		...
		std::copy(snap.EField    .begin(), snap.EField    .end(), &eField   [0][0]);
