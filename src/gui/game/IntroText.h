...
inline ByteString IntroTextMod()
{
	...
	sb << "\bu\bU" << APPNAME << "\bU\bl - Version " << DISPLAY_VERSION[0] << "." << DISPLAY_VERSION[1] << "\n"
	      "\n"
	      "\buClassical Electromagnetism + Multi-threading + Async Fields\n"
	      "\blMagnetic & electric fields, Lorentz force, dielectrophoresis, induction, and more.\n"
	      "Fields computed via FFT Poisson solvers with asynchronous worker threads.\n"
	      "\n"
	      "\buQuick Start:\n"
	      "\bgUse sim.asyncFields(true) in the console (press ~) for async field computation.\n"
	      "\bgUse sim.threads(N) for N-threaded particle updates (e.g. sim.threads(8)).\n"
